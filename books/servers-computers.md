...
## Minasmorgul — Primary Windows Workstation

| Field | Value |
|---|---|
| OS | Windows 11 |
| IP | 192.168.10.16 (static DHCP) |
| DNS | minasmorgul.lan |
| VLAN | 10 (trusted) |
| Users | aule, sung.us@outlook.com |

Primary everyday machine. Has SSH tunnel configured to palantir for Zyxel and
MikroTik UI access from VLAN10.

SSH config (`C:\Users\aule\.ssh\config` and `C:\Users\sungu\.ssh\config`):
```
Host palantir
...
Usage: `ssh palantir` (keep window open), then browse http://localhost:8080 (Zyxel)
