...
All Docker containers run on **Rivendell** (192.168.10.4), with the exception of the UniFi controller which still runs on Moria.

| Stack | Directory | Compose File | Key Containers |
|-------|-----------|-------------|----------------|
| AI stack | `/mnt/work/ai-stack/` | `docker-compose.yml` | ollama, litellm, openclaw, grafana, prometheus |
...
| Openclaw | `/data/compose/openclaw/` | Separate compose | openclaw gateway, tools |

**Note:** The openclaw compose is outside the `/mnt/work/` structure at `/data/compose/openclaw/`. This is a historical path.
...
Aulë is the family AI assistant, running on Rivendell via openclaw in the AI stack.
...
- Port: 11434
- Loaded models: qwen2.5:3b (num_ctx=32768)
- GPU: GTX 1060 6GB

**LiteLLM** is the proxy/routing layer between Aulë and the LLM backends. Config: `/mnt/work/ai-stack/config/litellm.yaml`

**openclaw** is the Aulë agent itself — the gateway, memory, tools, and interface to Telegram/Discord. Config: `/home/aule/.openclaw/`
...
# Restart Aulë only
docker restart hermes_gateway

# Full AI stack restart
cd /mnt/work/ai-stack && docker compose down && docker compose up -d

# View openclaw logs
docker logs hermes_gateway --tail 100
```
