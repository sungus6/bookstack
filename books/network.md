...
**Critical rule — Never exclude VLAN1 from any ports.** VLAN1 is used internally by the Zyxel regardless of the management VLAN setting. Excluding it has caused complete lockouts requiring factory reset.
...
AP SSH credentials are managed globally by the UniFi controller and pushed to all APs.
...
This is one of the most important MikroTik configuration items. Missing entries cause subtle connectivity failures.
...
**vlan20-iot is intentionally NOT in LAN.** IoT uses Google DNS directly and has no legitimate reason to reach MikroTik.
...
MikroTik handles local DNS for VLAN10 and VLAN99 machines. The `.lan` suffix resolves to local IPs (e.g., `moria.lan` → 192.168.10.6). IoT devices (VLAN20) use Google DNS (8.8.8.8) directly — they do not resolve `.lan` names.
...
SSH config at `C:\Users\sungu\.ssh\config` (or `C:\Users\aule\.ssh\config`):
...
1. Open PowerShell and run: `ssh palantir` (keep the window open)
2. Browse http://localhost:8080 → Zyxel management UI
3. Browse http://localhost:8081 → MikroTik web interface

To verify the tunnel is active: `netstat -an | findstr 8080` should show LISTENING.
...
palantir must always have WiFi disabled. If WiFi is active, routing breaks — SSH replies go out via WiFi instead of wired VLAN99.
...
If you can't reach palantir or VLAN99 is broken, connect any machine to **ether5-EMERGENCY** on the MikroTik with a static IP:
